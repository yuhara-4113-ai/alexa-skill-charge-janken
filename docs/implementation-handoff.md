...
「アレクサ、チャージじゃんけんしよう」「アレクサ、チャージじゃんけんで勝負」は、呼び出し名と `StartGameIntent` のサンプル発話を組み合わせて起動します。`StartGameIntent` から起動した場合も、`LaunchRequest` と同じ `AWAITING_ACTION`、第1ラウンド、Alexaの初手チャージで開始します。呼び出し名は増やさず、公開用の起動例はDeveloper ConsoleのシミュレータとEcho実機で認識を確認してから確定します。
