...
- ビーム、ファイアー、ブラックホールのパワーは、相手の行動や勝敗に関係なく消費する。
- 必要パワーに満たない技をユーザーが選んだ場合、そのラウンドは進めず、同じAlexaの手を隠したまま必要パワーを案内して再入力を求める。
- パワー上限と最大ラウンド数は設けない。
- ガードの連続使用制限も設けない。
...
- ビーム・ファイアー・ブラックホールは、いずれもチャージに勝つ。
- 技同士は、威力1のビーム、威力2のファイアー、威力3のブラックホールの順で強く、威力が高い側が勝つ。
- 同じ威力の技同士は勝敗なしで継続する。
- ガードはビームとファイアーを防ぐ。
- ブラックホールはガードを貫通して勝つ。
- 上記以外の組み合わせは勝敗なしで継続する。

### 3.3 Alexaの行動と公平性

- 第1ラウンドのAlexaの行動は必ず「チャージ」とする。
- 第2ラウンド以降は、その時点で選択可能な行動から一様ランダムに選ぶ。パワー0ではチャージ・ガード、1ではチャージ・ビーム・ガード、2ではチャージ・ビーム・ファイアー・ガード、3以上では全行動を候補とする。
...
2. Alexaが初回は「チャージじゃんけんへようこそ。チャージ、ビーム、ファイアー、ブラックホール、ガードのどれかを言ってね。せーの」、2ラウンド目以降は「せーの」と発話し、セッションを開いたままにする。
...
Alexaが自分の行動を読み上げるときは、チャージ、ビーム、ガード、ファイアー、ブラックホールを行動名のまま発声します。
...
type GameSession = {
  ...
  alexaWins: number;
  round: number;
  pendingAlexaAction?: 'charge' | 'attack' | 'fire' | 'blackhole' | 'defend';
};
```
...
|---|---|---|
| なし | `LaunchRequest` | 初回の選択肢を案内して `AWAITING_ACTION` |
...
5つの行動を別々のIntentにせず、1つの `ActionIntent` とカスタムスロットへ集約します。
...
`ACTION_TYPE` は、値のIDと同義語を次のように定義します。代表値はユーザーへ案内する技名に合わせ、旧呼称は入力互換の同義語として残します。

| ID | 代表値・同義語 |
|---|---|
| `charge` | チャージ、溜め、ためる、ため |
| `attack` | ビーム、ビーモ、攻撃、アタック |
| `fire` | ファイアー、ファイヤー、ファイア、ファイヤ |
| `blackhole` | ブラックホール |
| `defend` | ガード、防御、バリアー、バリア |

必要なIntentは次のとおりです。

- `ActionIntent`
- `StartGameIntent`（「スタート」「始める」「ゲームを始める」「しよう」「勝負」「勝負する」「遊ぶ」「対戦」）
...
`ActionIntent` はentity resolutionが `ER_SUCCESS_MATCH` の場合は解決済みIDを優先します。一致しない場合は、スロットの生値から空白を除去し、定義済みの行動別名を補完します。さらに「チャー…／ちゃー…」「ビー…／びー…」「ガー…／がー…」で始まる値は、それぞれチャージ、ビーム、ガードとして処理します。この前方一致は `ActionIntent` の行動入力だけに適用し、各接頭辞の対応先を固定するため、別の技には変換しません。どの技の接頭辞にも一致しない値は、Alexaの保留中の行動を維持して再入力を促します。
...
「アレクサ、チャージじゃんけんで勝負」「アレクサ、チャージじゃんけんで遊ぶ」「アレクサ、チャージじゃんけんで対戦」は、呼び出し名と `StartGameIntent` のサンプル発話を組み合わせて起動します。`StartGameIntent` から起動した場合も、`LaunchRequest` と同じ `AWAITING_ACTION`、第1ラウンド、Alexaの初手チャージで開始します。developmentのASKシミュレータでは、これら3件と標準起動の成功を確認済みです。「アレクサ、チャージじゃんけんしよう」はサンプル発話を追加してもシミュレータで成功しなかったためStore用の起動例には使用せず、Echo実機でのみ追加確認します。呼び出し名は増やしません。
...
- 25通りすべての行動組み合わせと勝敗。
- チャージの+1、ビームの-1、ファイアーの-2、ブラックホールの-3、ガードの変化なし。
- 技同士でも、それぞれの行動コストを両者が消費する。
- 技対ガードでは技側だけが行動コストを消費する。
- 必要パワーに満たない技は無効で、状態と `pendingAlexaAction` が変わらない。
- 第1ラウンドのAlexaは必ずチャージする。
- 第2ラウンド以降のAlexaが、現在のパワーで不可能な行動を選ばない。
- Alexaの行動選択がユーザーの現在の行動に依存しない。
- 再戦でパワーとラウンドは戻り、同一セッションの勝数は残る。
- phase外のIntent、Fallback、Help、Stopの応答。
- Skill ID不一致のリクエスト拒否。
- 再戦時の短い案内と勝数維持。
- entity resolution不一致時の既知の別名・技別前方一致フォールバックと、どの技にも対応しない未知語の拒否。
