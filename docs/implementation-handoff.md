...
- 攻撃系のパワーは、相手の行動や勝敗に関係なく消費する。
- 必要パワーに満たない攻撃系の行動をユーザーが選んだ場合、そのラウンドは進めず、同じAlexaの手を隠したまま必要パワーを案内して再入力を求める。
- パワー上限と最大ラウンド数は設けない。
- 防御の連続使用制限も設けない。
...
- 攻撃・ファイアー・ブラックホールは、いずれも溜めに勝つ。
- 攻撃系同士は、威力1の攻撃、威力2のファイアー、威力3のブラックホールの順で強く、威力が高い側が勝つ。
- 同じ威力の攻撃系同士は勝敗なしで継続する。
- 防御は攻撃とファイアーを防ぐ。
- ブラックホールは防御を貫通して勝つ。
...
- 第2ラウンド以降は、その時点で選択可能な行動から一様ランダムに選ぶ。パワー0では溜め・防御、1では溜め・攻撃・防御、2では溜め・攻撃・ファイアー・防御、3以上では全行動を候補とする。
...
2. Alexaが初回は「チャージじゃんけんへようこそ。溜め、攻撃、ファイアー、ブラックホール、防御のどれかを言ってね。せーの」、2ラウンド目以降は「せーの」と発話し、セッションを開いたままにする。
...
Alexaが自分の行動を読み上げるときは、溜めを「チャージ」、攻撃を「ビーム」、防御を「バリアー」と発声します。ファイアーとブラックホールは行動名をそのまま発声します。
...
- 25通りすべての行動組み合わせと勝敗。
- 溜めの+1、攻撃の-1、ファイアーの-2、ブラックホールの-3、防御の変化なし。
- 攻撃系同士でも、それぞれの行動コストを両者が消費する。
- 攻撃系対防御では攻撃側だけが行動コストを消費する。
- 必要パワーに満たない攻撃系の行動は無効で、状態と `pendingAlexaAction` が変わらない。
- 第1ラウンドのAlexaは必ず溜める。
