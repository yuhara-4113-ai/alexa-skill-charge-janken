...
`ActionIntent` はentity resolutionが `ER_SUCCESS_MATCH` の場合は解決済みIDを優先します。一致しない場合は、スロットの生値から空白を除去し、定義済みの行動別名を補完します。さらに「チャー…」「ビー…」「ガー…」で始まる値は、それぞれチャージ、ビーム、ガードとして処理します。この前方一致は `ActionIntent` の行動入力だけに適用し、各接頭辞の対応先を固定するため、別の技には変換しません。どの技の接頭辞にも一致しない値は、Alexaの保留中の行動を維持して再入力を促します。
