...
アイコンは文字、数字、AlexaやAmazonのロゴ、既存キャラクターを含みません。背景と四隅は透過させ、円形表示で四角い輪郭が見えないようにします。108px版は全辺16px、512px版は全辺75pxを目安に、中央の手とチャージ表現が円形の安全領域に収まる余白を確保します。108px版でも主要モチーフを識別でき、白背景・暗色背景のどちらでも十分なコントラストがあることを確認します。
...
- Store詳細説明が段落単位で読みやすく、ゲーム仕様・技名・課金等の記載が実装と一致すること
- 108px版と512px版が透過PNGであり、白背景・暗色背景それぞれの円形表示で四角い輪郭や主要モチーフの欠けがないこと
