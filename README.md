...
スキルは標準の「アレクサ、チャージじゃんけんを開いて」に加え、`StartGameIntent` を使う「アレクサ、チャージじゃんけんしよう」「アレクサ、チャージじゃんけんで勝負」からも起動できる対話モデルです。どちらから起動しても、初手をチャージに固定した同じ第1ラウンドへ進みます。公開前にDeveloper ConsoleのシミュレータとEcho実機で認識結果を確認します。
